...
\abstract{
We present state-of-the-art predictions for Standard Model Higgs production in association with a bottom-quark pair (\bbH{}). Updated cross-sections are computed in accordance with the recommendations of the LHC Higgs Working Group. Different flavour schemes can be adopted: a massless calculation with five active flavours (5FS) or a calculation with massive bottom quarks and four massless flavours (4FS). First, we provide matched predictions between flavour schemes at the fixed-order level using the FONLL method to combine N3LO 5FS and NLO 4FS results. \bbH{} involves both bottom-quark ($y_b$) and top-quark Yukawa ($y_t$) interactions. TOgether with their relevance in di-Higgs searches, we present NLO+PS results for the \(y_t^2\) contribution and NNLO+PS predictions for the \(y_b^2\) contribution in both massless and massive flavor schemes. We assess a Monte Carlo uncertainty by comparing the 5FS NNLO+PS predictions obtained with the \minnlo{} and \GENEVA{} methods. Finally, we study the sensitivity to light-quark Yukawa couplings in Higgs production using resummed analytic results and novel NNLO+PS simulations.
}
