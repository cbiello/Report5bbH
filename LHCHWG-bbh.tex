...
gluon-induced PDF channels which start at one higher order play a much more prominent role which explains the observed pattern of uncertainties for the different cases. Further, we want to note that the right panel of~\fig{fig:lightpTHzoom} is only shown for $p_T> 5\, \GeV$. With
the bottom mass $m_b = 4.18\, \GeV$ the assumption of $m_q \ll p_T$ and therefore our factorization
theorem are no longer valid for $p_T<5\, \GeV$.

\begin{figure}[t!]
\begin{center}
\begin{tabular}{cc}
	\includegraphics[width=.45\textwidth, page=1]{plots/5fs/light/ptHzoom_qqH.pdf}&
	\includegraphics[width=.45\textwidth, page=1]{plots/5fs/N3LLp_aN3LO/qqH-n3llp-an3lo.pdf}
\end{tabular}
\vspace*{1ex}
